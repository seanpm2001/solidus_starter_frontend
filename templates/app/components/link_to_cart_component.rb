...
class LinkToCartComponent < ViewComponent::Base
  ...
  def text
    empty_current_order? ? '' : "<div class='absolute flex -top-2 -right-2 items-center justify-center bg-red-500 h-5 w-5 p-0.5 rounded-full text-white text-body-sm md:text-body-2xs'>#{current_order.item_count}</div>"
  end
